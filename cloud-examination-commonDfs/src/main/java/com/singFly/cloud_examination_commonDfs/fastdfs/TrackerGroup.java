package com.singFly.cloud_examination_commonDfs.fastdfs;
...
public class TrackerGroup
{
	...
	// Logger.getLogger(TrackerGroup.class);
	...
	public TrackerServer getConnection() throws IOException
	{
		...
		{
			logger.error("connect to server " + this.tracker_servers[current_index].getAddress().getHostAddress() + ":"
		...
		for (int i = 0; i < this.tracker_servers.length; i++)
		{
			...
			{
				logger.error("connect to server " + this.tracker_servers[i].getAddress().getHostAddress() + ":"
